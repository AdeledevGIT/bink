...
function updatePreviewFrame(userId) {
    ...
    let previewUrl = `../u/${username}?t=${template}&preview=true`;
...
function saveBioLinkToDashboard() {
    ...
    const bioLinkData = {
        ...
        url: new URL(`u/${username}`, window.location.origin).href,
        platform: 'website',
        isBioLink: true,
...
function copyPreviewLinkToClipboard() {
    ...
    // This way the link will always show the user's current template
    const previewUrl = new URL(`u/${username}`, window.location.origin).href;
